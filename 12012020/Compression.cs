int Compression(char[] input){
	int slow = 0, fast = 0, curr = 0;

	while (fast < input.Length)
	{
		if(input[slow] != input[fast]){
			if(fast-slow == 1) input[curr] = input[slow];
			else{
				input[curr] = input[slow];
				curr++;
				input[curr] = Convert.ToChar(fast-slow);
			}
			curr+=1;
			slow=fast;
		}
...
//char[] input;
//input = new char[] {'a', 'a', 'a', 'a', 'a', 'a'};
//Console.WriteLine(Compression(input));
//Console.WriteLine(input);
//Console.WriteLine("------");
//input = new char[] {'a', 'a', 'b', 'b', 'c', 'c'};
//Console.WriteLine(Compression(input));
//Console.WriteLine(input);
//Console.WriteLine("------");
